...
const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4 relative">
      {/* Back to Home */}
      <Link to="/" className="absolute top-6 left-6 flex items-center gap-2 hover:opacity-80 transition-opacity z-10">
      ...
      <div className="bg-gray-800 border border-gray-700 p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="text-right">
                <Link to="/forgot-password" className="text-sm text-indigo-400 hover:text-indigo-300 transition-colors">Esqueci minha senha</Link>
            </div>
          </div>
        ...
        <p className="text-center text-gray-500 mt-8 text-sm">
            Ainda não tem conta? <Link to="/register" className="text-indigo-400 hover:text-indigo-300 font-semibold hover:underline transition-colors">Teste Grátis</Link>
        </p>
      </div>
    </div>
  );
