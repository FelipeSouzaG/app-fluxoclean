...
export function formatRegister(value: string): string {
  if (!value) return '';
...
export function validateRegister(value: string): boolean {
  const cleanValue = value.replace(/\D/g, '');
...
export function formatCurrencyNumber(value: any): string {
...
export function formatMoney(value: string): string {
  ...
  const floatValue = (parseInt(priceValue) / 100).toFixed(2);
  let valueMoney = `R$ ${floatValue.replace('.', ',')}`;
  return valueMoney;
}
...
export function checkPasswordStrength(password: string) {
  return {
    ...
    symbol: /[\W_]/.test(password), // Non-word characters (symbols)
  };
}
